...
export const PieChart: React.FC<Props> = ({ data, size, large = false }) => {
  ...
  const getPathData = ({
    ...
    const pathData = [
      ...
      `A ${larger} ${larger} 0 ${largeArcFlag} 1 ${endX * larger} ${endY * larger}`, // Arc
      percent === 1
        ? `A ${larger} ${larger} 0 ${largeArcFlag} 1 ${startX * larger} ${startY * larger}`
        : '', // Second half for 100%
      'L 0 0',
    ].join(' ');
    ...
    return {
      ...
      transform: largeSlice
        ? `${((endY - startY) / percent) * sliceSpace},${((startX - endX) / percent) * sliceSpace}`
        : '',
    };
  };
  return (
    ...
    >
      {data.map(({ color, percent, large: largeSlice }) => {
        ...
        return (
          <Path
            key={`${color}-${largeSlice ? 'large' : 'small'}`}
            d={slice.path}
            fill={color}
