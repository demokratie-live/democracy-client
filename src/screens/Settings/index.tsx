...
const DevModeButton = styled(TouchableOpacity)<{ devMode: boolean }>`
  ...
  width: 20px;
  height: 20px;
  background-color: ${({ theme, devMode }) =>
    devMode ? "red" : "transparent"};
`;
...
export const SettingsScreen: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      setAlreadyDenied(status !== "granted" && !canAskAgain);
    })();
  }, []);
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <DevModeButton devMode={devMode} onPress={toggleDevMode} />
      ),
    });
  ...
  const handleActivate = async () => {
    if (!alreadyDenied || Platform.OS === "android") {
      ...
      if (status !== "granted") {
        ...
        // Handle successful permission request if needed
      }
    } else {
      Alert.alert(
        "Benachrichtigungen",
