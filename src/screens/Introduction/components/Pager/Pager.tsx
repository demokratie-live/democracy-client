...
function hasButton(props: PropsBase | PropsWithButton): props is PropsWithButton {
...
export const Pager: FC<Props> = props => {
  ...
  return (
    <>
      ...
      >
        {children.map(child => React.cloneElement<PageProps>(child, { nextPage }))}
      </ScrollView>
      {hasButton(props) && (
        <NextButton
      ...
      <Dots length={length} current={currentDot} withButton={hasButton(props)} />
    </>
  );
};
