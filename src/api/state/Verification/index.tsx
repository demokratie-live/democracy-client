...
const defaults: InitialStateInterface = {
  phoneNumber: '',
  setPhoneNumber: () => {
    throw new Error('VerificationContext: setPhoneNumber function is not defined');
  },
  countdown: 0,
  setResendTime: () => {
    throw new Error('VerificationContext: setResendTime function is not defined');
  },
  setExpireTime: () => {
    throw new Error('VerificationContext: setResendTime function is not defined');
  },
...
export const VerificationContext = createContext<InitialStateInterface>(defaults);
...
export const VerificationProvider: FC = ({ children }) => {
  ...
  useEffect(() => {
    let countdownValue = Math.ceil((resendTime.getTime() - new Date().getTime()) / 1000);
    if (countdownValue > 0) {
      const countdownInterval = setInterval(() => {
        countdownValue = Math.ceil((resendTime.getTime() - new Date().getTime()) / 1000);
