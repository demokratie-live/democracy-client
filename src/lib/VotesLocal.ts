...
export type SetLocalVote = Pick<ChainEntry, 'procedureId' | 'selection' | 'constituency'>;
...
class VotesLocal {
  ...
  */
  ...
  static readKeychain = async (): Promise<Chain> => {
    ...
    if (
      !keyindex ||
      (keyindex && !VotesLocal.validateKeychain(JSON.parse(keyindex.password) as Chain))
    ) {
    ...
    if (indexchain.i) {
      await Promise.all(
        indexchain.i.map(async (serviceId: number) => {
          ...
          if (setData) {
            indexchain.d?.push(...(JSON.parse(setData.password) as ChainEntryRaw[]));
          }
        }),
      );
    }
  ...
  private static convertKeychainVersion0ToVersion1 = async () => {
    ...
    const newChain = oldChainKeyValue.reduce<Chain>(
      (prev, currentValue) => {
        ...
        return {
          ...prev,
          d: [...(prev.d || []), { i: key, s: value, t: new Date(0).toISOString(), c: null }],
        };
      },
  ...
  // Write VoteData
  static setVote = async ({ procedureId, selection, constituency }: SetLocalVote) => {
    const chain = await VotesLocal.readKeychain();
