import styled, { css, useTheme, ThemeContext } from 'styled-components/native';
export { ThemeProvider } from 'styled-components/native';
import * as React from 'react';
import React__default, { useState, useEffect, useContext } from 'react';
import Svg$1, { Defs, Path, Use, Svg as Svg$2, G, Text, Rect } from 'react-native-svg';
...
const Button$1 = styled.TouchableOpacity.withConfig({ displayName: "Button", componentId: "sc-4ecn3o" }) `
  width: 100%;
  height: 40px;
...
`;
const Label = styled.Text.withConfig({ displayName: "Label", componentId: "sc-b4s8hs" }) `
...
const CircleButton$1 = ({ color, size, children, ...props }) => (React__default.createElement(Circle, Object.assign({}, props, { color, size }), children));

const CircleButton = styled(CircleButton$1).withConfig({ displayName: "CircleButton", componentId: "sc-1hlhuv" }) `
  justify-content: center;
  align-items: center;
`;
const ThumbUp = styled(ThumbUpIcon).attrs({
    width: 45,
    height: 45,
...
const VoteButton = ({ decision }) => {
    ...
    return (React__default.createElement(CircleButton, { size: 88, color: backgroundColor },
        React__default.createElement(ThumbUp, { color: theme.colors.text.secondary, rotation: rotation })));
};

const Number = styled.Text.withConfig({ displayName: "Number", componentId: "sc-1qht293" }) `
  color: ${({ voted, theme }) => voted ? theme.colors.text.colored : theme.colors.text.primary};
  font-weight: bold;
`;
const VotesIndex = ({ votes, voted }) => (React__default.createElement(Number, { voted: voted }, votes));

const Svg = styled(Svg$2).withConfig({ displayName: "Svg", componentId: "sc-12ztyip" }) `
  flex: 1;
  align-self: stretch;
`;
const PieChart = ({ size, data }) => {
    ...
    const viewBox = Platform.OS === 'web'
        ? `-${size / 2} -${size / 2} ${size} ${size}`
        : undefined;
    return (React__default.createElement(Svg, { width: size, height: size, viewBox: viewBox },
        React__default.createElement(G, { x: size / 2, y: size / 2 }, paths.map((item, i) => item ? (React__default.createElement(Path, { key: data[i].name, fill: data[i].color, strokeWidth: size / 100, d: item })) : null))));
};
...
const DateText = styled.Text.withConfig({ displayName: "DateText", componentId: "sc-mnzu45" }) `
  color: ${({ theme, type }) => {
    switch (type) {
        case 'future':
...
const VoteDate = ({ date, dateEnd }) => {
    ...
    if (dateEnd && date <= new Date() && dateEnd >= new Date()) {
        return React__default.createElement(DateText, { type: 'current' }, "l\u00E4uft");
    }
    return (React__default.createElement(DateText, { type: date <= new Date() ? 'past' : 'future' }, timeLeft));
};

const CommunityChart = ({ communityChart, }) => communityChart ? (React__default.createElement(React__default.Fragment, null, React__default.isValidElement(communityChart) ? (communityChart) : (React__default.createElement(PieChart, Object.assign({}, communityChart))))) : null;

const GovernmentChart = ({ governmentChart, }) => governmentChart ? (React__default.createElement(React__default.Fragment, null, React__default.isValidElement(governmentChart) ? (governmentChart) : (React__default.createElement(PieChart, Object.assign({}, governmentChart))))) : null;

const ProcedureListItem$1 = styled.View.withConfig({ displayName: "ProcedureListItem", componentId: "sc-1tq0vzw" }) `
  flex-direction: row;
  padding-horizontal: ${({ theme }) => theme.spaces.default};
  padding-vertical: ${({ theme }) => theme.spaces.default};
`;
const TextContainer = styled.View.withConfig({ displayName: "TextContainer", componentId: "sc-18tdyl4" }) `
  flex: 1;
  justify-content: space-between;
`;
const Title = styled.Text.withConfig({ displayName: "Title", componentId: "sc-muzs0q" }) `
  font-size: 17px;
  color: ${({ theme }) => theme.colors.text.primary};
`;
const Subline = styled.Text.withConfig({ displayName: "Subline", componentId: "sc-10us3hf" }) `
  padding-top: 8px;
  font-size: 15px;
  color: ${({ theme }) => theme.colors.text.tertiary};
`;
const SideContainer = styled.View.withConfig({ displayName: "SideContainer", componentId: "sc-1esmvln" }) `
  align-items: flex-end;
  min-width: 50px;
  justify-content: space-between;
`;
const PieChartContainer = styled.View.withConfig({ displayName: "PieChartContainer", componentId: "sc-1dlw1ce" }) `
  flex-direction: row;
  width: 45px;
  justify-content: space-between;
`;
styled.Text.withConfig({ displayName: "Label", componentId: "sc-srmc88" }) `
  font-size: ${({ theme }) => theme.fontSizes.default};
  text-transform: uppercase;
  font-weight: 500;
  letter-spacing: 1px;
...
const CountryMap = ({ data }) => {
    ...
    const maxVotersConstituency = max(allConstituencies.constituencies.map(({ total }) => total));
    return (React__default.createElement(Container$1, null,
        React__default.createElement(CountryMapSvg, { width: width / 1.5 }, Object.keys(constituencySvgs).map((constituencyI) => {
            ...
            return (React__default.createElement(G, { key: `constituency-${constituency}`, opacity: total ? opacityRange(total / (maxVotersConstituency || 0)) : 0.5 }, constituencySvgs[constituency]({
                fill: total
...
const VoteResultsWrapper = styled.View.withConfig({ displayName: "VoteResultsWrapper", componentId: "sc-wcoo67" }) `
  align-items: center;
`;
const VoteResultNumbers = styled.View.withConfig({ displayName: "VoteResultNumbers", componentId: "sc-18j7tuh" }) `
  ...
  height: 40px;
`;
const VoteResult = styled.View.withConfig({ displayName: "VoteResult", componentId: "sc-phsgec" }) `
  justify-content: center;
  align-items: center;
`;
const VoteResultCircleNumber = styled.View.withConfig({ displayName: "VoteResultCircleNumber", componentId: "sc-1jb9e9n" }) `
  flex-direction: row;
`;
const VoteResultNumber = styled.Text.withConfig({ displayName: "VoteResultNumber", componentId: "sc-1t3kwod" }) `
  color: ${({ theme }) => theme.colors.text.primary};
  font-size: 12px;
  padding-top: 1px;
`;
const VoteResultLabel = styled.Text.withConfig({ displayName: "VoteResultLabel", componentId: "sc-1jt1k5p" }) `
  color: ${({ theme }) => theme.colors.text.primary};
  font-size: 12px;
  padding-top: 6px;
`;
const VoteResultCircle = styled.View.withConfig({ displayName: "VoteResultCircle", componentId: "sc-180xkin" }) `
  width: 11px;
  height: 11px;
  border-radius: 5px;
  background-color: ${({ color }) => color};
...
const ChartLegend = ({ data }) => {
    return (React__default.createElement(VoteResultsWrapper, null,
        React__default.createElement(VoteResultNumbers, null, data.map(({ label, value, color }) => (React__default.createElement(VoteResult, { key: label },
            React__default.createElement(VoteResultCircleNumber, null,
                React__default.createElement(VoteResultCircle, { color: color }),
                React__default.createElement(VoteResultNumber, null, value)),
            React__default.createElement(VoteResultLabel, null, label)))))));
};

const Container = styled.View.withConfig({ displayName: "Container", componentId: "sc-1x5x0pp" }) `
  margin-horizontal: ${({ theme }) => theme.spaces.default};
  margin-vertical: ${({ theme }) => theme.spaces.default};
  align-items: center;
`;
const TopContainere = styled.View.withConfig({ displayName: "TopContainere", componentId: "sc-1lxhwgn" }) `
  position: absolute;
  flex-direction: row;
  justify-content: space-between;
  left: 0;
  right: 0;
`;
const TopLeftText = styled.Text.withConfig({ displayName: "TopLeftText", componentId: "sc-12cck09" }) `
  color: ${({ theme }) => theme.colors.text.primary};
`;
...
const DonutChart = ({ votesData, colors, innerTextTop, innerTextBottom, size, topLeftText, topRightSvg, hidePercentage = false, }) => {
    ...
    const vetColors = scaleOrdinal().domain(dataLabels).range(colors);
    const viewBox = Platform.OS === 'web'
        ? `-${chartSize / 2} -${chartSize / 2} ${chartSize} ${chartSize}`
        : undefined;
    return (React__default.createElement(Container, null,
        !!topLeftText && (React__default.createElement(TopContainere, null,
            React__default.createElement(TopLeftText, null, topLeftText),
            topRightSvg)),
        React__default.createElement(Svg$2, { width: chartSize, height: chartSize, viewBox: viewBox },
            React__default.createElement(G, { x: chartSize / 2, y: chartSize / 2 }, 
            // pieChart has all the svg paths calculated in step 2)
            pieEntryData.map(({ path, textTransform, percentage }, index) => path ? (React__default.createElement(G, { key: 'pie_shape_' + index },
                React__default.createElement(Path, { fill: vetColors(preparedData[index].name), strokeWidth: chartSize / 100, d: path }),
                !hidePercentage && percentage > 3 && (React__default.createElement(Text, { y: "6", transform: `translate(${textTransform[0]} ${textTransform[1]})`, fontSize: '18', textAnchor: "middle", fill: theme.colors.text.secondary }, `${percentage}%`)))) : null)),
            React__default.createElement(G, { x: chartSize / 2, y: chartSize / 2 },
                React__default.createElement(Text, { y: -4, fontSize: "18", textAnchor: "middle", fill: theme.colors.text.primary }, innerTextTop),
                React__default.createElement(Text, { y: 15, fontSize: "14", textAnchor: "middle", fill: theme.colors.text.primary }, innerTextBottom)))));
};
...
const Bar = ({ data, width, height, active = true, }) => {
    ...
    const getPercentage = (value, { x } = {}) => {
        ...
        return (React__default.createElement(Text, { x: (xScale(value) || 0) + (x || 0) - 4, y: height / 2 + 5, fontSize: 14, textAnchor: "end", fill: themeContext.colors.text.secondary }, `${percentage}%`));
    };
    let x = 0;
    return (React__default.createElement(G, { opacity: active ? 1 : 0.5 }, data.map(({ value, color }) => {
        x = x + xScale(value);
        return (React__default.createElement(G, { transform: `translate(${-xScale(value) + x} 0)` },
            React__default.createElement(Rect, { width: xScale(value), height: height, fill: color }),
            getPercentage(value)));
    })));
};
...
const BarChart = ({ data, size, selectedParty, setSelectedParty, }) => {
    ...
    const yScale = scaleBand()
        ...
        .range([0, innerHeight])
        .padding(0.2);
    return (React__default.createElement(Svg$1, { width: size, height: size - 13 },
        React__default.createElement(G, { y: margin.top + 3 }, data.map(({ party }, i) => (React__default.createElement(Text, { opacity: i === selectedParty ? 1 : 0.5, key: `axis-${party}`, y: yScale.bandwidth() +
                ((yScale(party) || 0) - yScale.bandwidth() / 2), fill: theme.colors.text.primary }, party)))),
        data.map(({ party, deviants }, i) => (React__default.createElement(G, { key: `bar-${party}`, transform: `translate(${margin.left} ${yScale(party) || 0})`, onPress: () => setSelectedParty(i) },
            React__default.createElement(Bar, { active: i === selectedParty, data: deviants, width: innerWidth, height: yScale.bandwidth() }))))));
};

export { BarChart, Button, ChartLegend, CountryMap, DonutChart, PieChart, ProcedureListItem, ThumbUpIcon, VoteButton, VoteDate, VotesIndex, darkTheme, lightTheme };
